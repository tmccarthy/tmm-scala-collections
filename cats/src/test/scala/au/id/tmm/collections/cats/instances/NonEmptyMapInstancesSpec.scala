...
import au.id.tmm.collections.cats.instances.NonEmptyMapInstancesSpec.EqOnlyInt
...
import cats.laws.discipline.{AlignTests, FlatMapTests, FunctorFilterTests, FunctorTests, UnorderedTraverseTests}
import cats.tests.CatsSuite
import org.scalacheck.{Arbitrary, Cogen, Shrink}

class NonEmptyMapInstancesSpec extends CatsSuite {

  checkAll("Eq for tmmUtils NonEmptyMap", EqTests[NonEmptyMap[String, EqOnlyInt]].eqv)
  ...
  checkAll("Align for tmmUtils NonEmptyMap", AlignTests[NonEmptyMap[String, *]].align[Int, Int, Int, Int])
  checkAll(
    "FunctorFilter for tmmUtils NonEmptyMap",
    FunctorFilterTests[NonEmptyMap[String, *]].functorFilter[Int, Int, Int],
  )
  checkAll(
    "CommutativeSemigroup for tmmUtils NonEmptyMap",
    CommutativeSemigroupTests[NonEmptyMap[String, Int]].commutativeSemigroup,
  )
  checkAll("Semigroup for tmmUtils NonEmptyMap", SemigroupTests[NonEmptyMap[String, Int]].semigroup)
  checkAll(
...
object NonEmptyMapInstancesSpec {
